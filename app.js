...
function afterLog() {
  ...
  document.querySelectorAll('button[data-month-nav]').forEach(b => {
    b.onclick = () => {
      const base = /^\d{4}-\d{2}$/.test(logFilter.month || '')
        ? logFilter.month
        : (document.querySelector('.cal-title')?.textContent || '').replace(/(\d{4})年(\d{1,2})月/, (mAll, yy, mm) => `${yy}-${String(mm).padStart(2, '0')}`) || todayStr().slice(0, 7);
